...
function startComputerThrow() {
    ...
    const shuffleInterval = setInterval(() => {
        ...
        computerImage.src = `/Homework/hw4/resources/${choices[randomChoice].toLowerCase()}.png`;

        shuffleCount++;
        if (shuffleCount >= 6) { 
            clearInterval(shuffleInterval);
            computerChoice = Math.ceil(Math.random() * 3); 
            computerImage.src = `/Homework/hw4/resources/${choices[computerChoice].toLowerCase()}.png`;
            gameLoop();
        }
    }, 500);
}
...
document.getElementById("reset").addEventListener('click', () => {
    ...
    document.getElementById("computer-choice-image").src = "/Homework/hw4/resources/question-mark.png";
